...
parser = argparse.ArgumentParser(
    description='Domain manager for http://pdd.yandex.ru service')
parser.add_argument('action', help='add or remove')
parser.add_argument('domain', help='domain name')
parser.add_argument('--ip', help='Domain ip')
parser.add_argument('--ttl', default=360,
                    help='Ttl dor domain. default is 360 seconds')
...
def add(domain_name, subdomain_name, ttl, ip):
    params = {'domain': domain_name, 'type': 'A',
              'subdomain': subdomain_name, 'ttl': ttl, 'content': ip}
...
def __get_record_id(domain_name, fqdn):
    ...
    if response.status_code not in range(200, 300) \
            or json.loads(response.text)['success'] == 'error':
        print response.text
        exit(2)
